// scripts/check-vat-fixed.ts
// Verify VAT calculation with the fix
...
async function main() {
    ...
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║     CHECK VAT DATA (WITH FIX)                                ║');
    console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    for (const t of purchases) {
        // Try items first
        let goodsValue = t.items.reduce((sum, i) => sum + Number(i.sub_total || i.line_total || 0), 0);
        let vatAmount = t.items.reduce((sum, i) => sum + Number(i.vat_amount || i.tax_amount || 0), 0);

        // Fallback to header
        if (vatAmount === 0) {
            vatAmount = Number(t.vat_amount || t.tax_amount || 0);
        }
        if (goodsValue === 0) {
            goodsValue = Number(t.subtotal || t.amount || 0);
        }
        ...
        if (vatAmount > 0) {
            ...
            console.log(`    Goods: ${goodsValue.toLocaleString()}đ, VAT: ${vatAmount.toLocaleString()}đ (from header)`);
        }
    }
